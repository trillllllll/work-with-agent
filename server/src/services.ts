...
export class TaskService {
  async list(topicId?: string, options: { includeDeleted?: boolean; inbox?: boolean } = {}) { return prisma.task.findMany({ where: { ...(topicId ? { topicId } : {}), ...(options.inbox ? { topicId: null } : {}), ...(options.includeDeleted ? {} : { deletedAt: null }) }, orderBy: { updatedAt: 'desc' } }); }
  ...
  async create(input: { topicId?: string | null; title: string; description?: string; status?: TaskStatus; priority?: TaskPriority; dueDate?: string | null; resultSummary?: string }, context: MutationContext | string = { source: 'user' }) { const ctx = typeof context === 'string' ? { source: context as MutationContext['source'] } : context; return prisma.$transaction(async (tx) => { if (input.topicId && !(await tx.topic.findUnique({ where: { id: input.topicId } }))) throw notFound('主题不存在'); const timestamp = now(); const result = await tx.task.create({ data: { topicId: input.topicId ?? null, title: input.title, description: input.description ?? '', status: input.status ?? TaskStatus.todo, priority: input.priority ?? TaskPriority.none, dueDate: input.dueDate ?? null, resultSummary: input.resultSummary ?? '', createdAt: timestamp, updatedAt: timestamp } }); await recordChange(tx, 'task', result.id, 'create', null, result, ctx); return result; }); }
  async update(taskId: string, input: Partial<{ title: string; description: string; status: TaskStatus; priority: TaskPriority; dueDate: string | null; resultSummary: string; topicId: string | null }>, context: MutationContext | string = { source: 'user' }) { const ctx = typeof context === 'string' ? { source: context as MutationContext['source'] } : context; try { return await prisma.$transaction(async (tx) => { if (input.topicId && !(await tx.topic.findUnique({ where: { id: input.topicId } }))) throw notFound('主题不存在'); const before = await tx.task.findUnique({ where: { id: taskId } }); if (!before) throw notFound('任务不存在'); if (before.deletedAt) throw Object.assign(new Error('回收站中的任务不能直接修改'), { status: 409 }); const result = await tx.task.update({ where: { id: taskId }, data: { ...input, updatedAt: now() } }); await recordChange(tx, 'task', taskId, 'update', before, result, ctx); return result; }); } catch (error: any) { if (error?.code === 'P2025') throw notFound('任务不存在'); throw error; } }
