...
const base = new URL(process.env.WWA_API_URL ?? 'http://127.0.0.1:3016');
...
async function request(path: string, method = 'GET', body?: unknown) {
  try {
    const response = await fetch(new URL(path, base), { method, headers: { Authorization: `Bearer ${credential}`, 'Content-Type': 'application/json' }, ...(body === undefined ? {} : { body: JSON.stringify(body) }), signal: AbortSignal.timeout(30_000), redirect: 'error' });
...
server.registerTool('list_memories', { description: 'Read scoped project memories with source health and lifecycle state.', inputSchema: { ...pagination, topicId: id.optional(), history: z.boolean().optional() } }, (args) => query('/api/v1/knowledge/memories', args));
