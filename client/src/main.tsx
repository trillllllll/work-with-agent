  ...
  return (
    <main className="app-shell">
      <aside className="sidebar">
        ...
        <div className="sidebar-heading"><span>主题</span><button className="icon-button" title="新建主题" onClick={() => setTopicForm({ name: '', description: '', isExploration: true })}>+</button></div>
        <div className="topic-list">
          {topics.map((topic) => <div className={`topic-row ${topic.id === selectedTopicId ? 'selected' : ''}`} key={topic.id}><button className="topic-button" onClick={() => setSelectedTopicId(topic.id)}><span className="topic-dot" />{topic.name}{topic.isExploration && <em>探索</em>}</button><button className="more-button" title="编辑主题" onClick={() => setTopicForm(topic)}>···</button></div>)}
          {!topics.length && !topicsQuery.isLoading && <p className="muted">还没有主题，先创建一个。</p>}
        </div>
        <div className="sidebar-footer">本地工作区</div>
      </aside>
      <section className="workspace">
        ...
        {!selectedTopic && <div className="empty-state"><div className="empty-icon">✦</div><h2>从一个主题开始</h2><p>主题是任务逐渐收敛成结果的容器。</p><button className="primary" onClick={() => setTopicForm({ name: '', description: '', isExploration: true })}>创建探索主题</button></div>}
      ...
      {topicForm && <TopicModal form={topicForm} onChange={setTopicForm} onClose={() => setTopicForm(null)} onSave={() => saveTopic.mutate(topicForm)} />}
      {taskForm && <TaskModal form={taskForm} onChange={setTaskForm} onClose={() => setTaskForm(null)} onSave={() => saveTask.mutate(taskForm)} />}
    </main>
  );
}
