...
import { TaskDetailModal, TaskDetailPanel, type TaskDetailHandle } from './components/board/TaskDetailModal.js';
...
export function App() {
  ...
  const openTask = (id: string) => {
    if (id === taskId) { setChatOpen(false); return; }
  ...
  const detailProps = taskId ? { id: taskId, topics, tags, onClose: () => setTaskId(''), onOpen: openTask, onSave: patchTask, onCreate: createTask, onToggle: toggleTask, onMove: moveTask, onDelete: (task: Task) => run(deleteTask(task)), onReorder: reorder } : null;
  ...
  return <>
    ...
    {!isDesktop && detailProps && <TaskDetailModal key={taskId} ref={detailRef} {...detailProps} />}
