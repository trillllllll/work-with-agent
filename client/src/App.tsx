...
import { Archive, Bot, CalendarClock, CalendarDays, ClipboardCheck, FolderKanban, History, Inbox, Library, Menu, MessageSquare, Plug, Plus, Search, Settings, Tags, Trash2 } from 'lucide-react';
...
export function App() {
  ...
  const [dragHintSeen, setDragHintSeen] = useState(() => localStorage.getItem('todo.dragHintSeen') === '1');
  ...
  const openChat = () => { setChatOpen(true); if (!isDesktop) navigate('chat'); };
  const markDragHint = () => { localStorage.setItem('todo.dragHintSeen', '1'); setDragHintSeen(true); };
  ...
  const todayGroups = [{ name: '逾期', items: tasks.filter((task) => task.dueDate && task.dueDate < today) }, { name: '今天', items: tasks.filter((task) => task.dueDate === today) }].filter((group) => group.items.length);
  const showDragHint = !hasFilters && !archived && sortMode === 'manual' && (!dragHintSeen || tasks.length === 0);
  const taskContent = <div className="mx-auto max-w-5xl p-4 pb-10 sm:p-6">
    <header className="mb-5 border-b pb-4 glass-divider"><div className="flex flex-wrap items-center justify-between gap-3"><div className="min-w-0"><h1 className="text-xl font-semibold tracking-tight">{title}</h1>{page === 'inbox' && <p className="mt-1 max-w-xl text-xs text-muted-foreground">这是一个分类，这里只显示放在收集箱里的任务。</p>}{page === 'today' && <p className="mt-1 text-xs text-muted-foreground">{today} · 未完成的到期任务</p>}{archived && <p className="mt-1 text-sm text-muted-foreground">此清单已归档，内容只读。</p>}</div><div className="flex flex-wrap items-center gap-2"><Button variant="ghost" size="icon" aria-label="打开聊天" onClick={openChat}><MessageSquare /></Button>{page === 'board' && detail && !archived && <Button variant="outline" aria-label={`编辑清单：${detail.name}`} onClick={() => setTopicForm(detail)}>编辑清单</Button>}{scoped && <div className="glass-control flex rounded-xl p-1"><Button variant={layout === 'list' ? 'secondary' : 'ghost'} size="sm" onClick={() => setLayout('list')}>列表</Button><Button variant={layout === 'board' ? 'secondary' : 'ghost'} size="sm" onClick={() => setLayout('board')}>看板</Button></div>}{scoped && !archived && <Button onClick={() => captureRef.current?.focus()}><Plus />新建任务</Button>}</div></div></header>
    ...
    {page === 'board' && !detail ? <Button onClick={newTopic}>新建清单</Button> : <>
      ...
      {scoped && (hasFilters || archived || sortMode !== 'manual' || showDragHint) && <p className="mb-3 text-xs text-muted-foreground">{hasFilters ? '筛选期间不能手动排序；清除筛选后可调整完整列表。' : archived ? '恢复清单后可编辑和排序。' : sortMode !== 'manual' ? '切换为手动排序后，可调整任务位置。' : '上下拖动调整顺序，向右拖成子任务，向左拖回根任务。'}</p>}
      {tasksQuery.isLoading ? <p className="py-12 text-center text-muted-foreground">正在加载任务…</p> : tasksQuery.error ? <p role="alert" className="text-destructive">{tasksQuery.error.message}</p> : page === 'today' ? (todayGroups.length ? <div className="space-y-6">{todayGroups.map((group) => <section key={group.name}><h2 className="mb-3 text-sm font-semibold">{group.name} · {group.items.length}</h2><TaskList {...listProps} tasks={group.items} /></section>)}</div> : <p className="py-12 text-center text-sm text-muted-foreground">暂无任务</p>) : scoped && layout === 'board' ? <div className="mt-5 flex gap-3 overflow-x-auto pb-4 snap-x snap-mandatory md:grid md:grid-cols-2 md:overflow-visible xl:grid-cols-4">{statuses.map(({ value, label }) => <BoardColumn key={value} label={label} count={tasks.filter((task) => task.status === value).length}>{tasks.filter((task) => task.status === value).map((task) => <TaskCard key={task.id} task={task} topics={topics} tags={tags} selected={taskId === task.id} readonly={archived} busy={actions.pending} textDirty={detailDirty} onOpen={openTask} onToggle={toggleTask} onMove={moveTask} onDelete={(item) => run(deleteTask(item))} onSave={patchTask} onCreate={createTask} onUpdateStatus={(id, status: Status) => { const item = tasks.find((candidate) => candidate.id === id); if (item) run(patchTask(item, { status })); }} />)}</BoardColumn>)}</div> : page === 'tags' ? (selectedTag ? <div className="space-y-6">{taggedSections.map((section) => <section key={section.id} aria-label={section.name}><h2 className="mb-3 text-sm font-semibold">{section.name} · {section.items.length}</h2><TaskList {...listProps} tasks={section.items} /></section>)}{!taggedSections.length && <p className="py-12 text-center text-sm text-muted-foreground">暂无任务</p>}</div> : <p className="py-12 text-center text-sm text-muted-foreground">选择一个标签后，按分类查看带有该标签的任务。</p>) : <TaskList {...listProps} tasks={tasks} foldDone={scoped && !hasFilters} hierarchical={scoped && !hasFilters && sortMode === 'manual'} onDragSettled={markDragHint} onReorder={scoped && !hasFilters && !archived && sortMode === 'manual' ? reorder : undefined} onNest={scoped && !hasFilters && !archived && sortMode === 'manual' ? nest : undefined} />}
    </>}
  </div>;
