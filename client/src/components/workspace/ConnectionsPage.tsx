...
type ConnectionConfiguration = { command: string; args: string[]; url: string; env: { WWA_API_URL: string; WWA_CONNECTION_TOKEN: string } };
...
export function ConnectionsPage({ topics }: { topics: Topic[] }) {
  ...
  return <section className="mx-auto max-w-4xl space-y-6 p-5 sm:p-7">
    <header><h1 className="text-2xl font-semibold">AI 连接</h1><p className="mt-2 text-sm text-muted-foreground">让 Codex、Claude Code 和 Grok 访问选定清单。每个连接拥有独立凭据，默认修改需要确认。</p></header>
...
function ConnectionSetup({ connection, onDone }: { connection: CreatedConnection; onDone: () => void }) {
  ...
  const quotedArgs = config.args.map(quotePowerShell).join(' ');
  const apiEnv = quotePowerShell(`WWA_API_URL=${config.env.WWA_API_URL}`);
  const tokenEnv = quotePowerShell(`WWA_CONNECTION_TOKEN=${config.env.WWA_CONNECTION_TOKEN}`);
  const claudeCommand = `claude mcp add work_with_agent ${quotePowerShell(config.command)} ${quotedArgs} --scope user --env ${apiEnv} ${tokenEnv}`;
  const grokCommand = `grok mcp add work_with_agent --scope user --transport http ${config.url} --header ${quotePowerShell(`Authorization: Bearer ${config.env.WWA_CONNECTION_TOKEN}`)}`;
  ...
  const setup = connection.host === 'claude' ? claudeCommand : connection.host === 'grok' ? grokCommand : codexConfig;
  const label = hostLabel(connection.host);
  const copy = async () => {
  ...
  return <section className="glass-subtle space-y-4 rounded-xl border border-primary/30 p-4">
    <div><h2 className="font-semibold">连接凭据</h2><p className="mt-1 text-sm text-muted-foreground">这是 {label} 访问本应用的钥匙，只在本次创建后显示。</p></div>
    <Input readOnly aria-label="连接凭据" value={connection.token} />
    <ol className="list-decimal space-y-4 pl-5 text-sm">
      <li><p className="mb-2">确认本应用服务保持运行，然后在 PowerShell {connection.host === 'codex' ? '将下面内容加入 Codex 的 config.toml' : '执行下面这条命令'}：</p><pre className="overflow-x-auto whitespace-pre-wrap break-all rounded-lg bg-muted p-3 text-xs">{setup}</pre><Button className="mt-2" type="button" variant="outline" size="sm" onClick={() => void copy()}>{copied ? '已复制' : `复制 ${connection.host === 'grok' ? 'Grok 配置命令' : connection.host === 'claude' ? 'Claude Code 配置命令' : 'Codex 配置'}`}</Button></li>
      <li>{connection.host === 'grok' ? <><p>运行以下命令确认配置已保存：</p><pre className="mt-2 rounded-lg bg-muted p-3 text-xs">grok mcp list</pre></> : connection.host === 'claude' ? <><p>运行以下命令确认配置已保存：</p><pre className="mt-2 rounded-lg bg-muted p-3 text-xs">claude mcp get work_with_agent</pre></> : <p>重新启动 Codex，并在 MCP 面板确认 <code>work_with_agent</code> 已启用。</p>}</li>
      <li><p>在新的 {label} 会话中测试：</p><pre className="mt-2 whitespace-pre-wrap rounded-lg bg-muted p-3 text-xs">请使用 work_with_agent 的 list_tasks 列出我的收集箱任务。</pre></li>
    </ol>
