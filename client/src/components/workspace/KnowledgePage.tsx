...
import { KnowledgeTree } from './KnowledgeTree.js';
import { asLabels, MemorySummary, memoryKindNames, originNames, Stars } from './memory-view.js';

type PageResult<T> = { items: T[]; total: number; nextCursor: number | null };
type Resource = { id: string; title: string; kind: string; revision: number; content?: string; uri?: string; health?: string; status?: string; evidence?: any[]; archivedAt?: string | null; versions?: any[]; importance?: number; labels?: string[]; origin?: string; createdAt?: string; updatedAt?: string };
type ResourceEditing = { id: string; scope: string; resource?: Resource };
type BriefDraft = { notes: string; revision: number };
type KnowledgeTab = 'materials' | 'memories' | 'brief' | 'search' | 'tree' | 'graph';
...
export function KnowledgePage({ topics, initialTopicId = '', taskId }: { topics: Topic[]; initialTopicId?: string; taskId?: string }) {
  ...
  const [tab, setTab] = useState<KnowledgeTab>('materials');
  const [cursor, setCursor] = useState(0);
  const [search, setSearch] = useState('');
  const [memoryQuery, setMemoryQuery] = useState('');
  const [memoryStatus, setMemoryStatus] = useState<'active' | 'history' | 'all'>('active');
  ...
  const list = useQuery({ queryKey: ['knowledge', tab, topicId, taskId, cursor, history, memoryStatus, memoryQuery], queryFn: () => api<PageResult<Resource>>(`/api/v1/knowledge/${tab}?${scope}&cursor=${cursor}&history=${history}&includeArchived=${history}${taskId && tab === 'materials' ? `&taskId=${taskId}` : ''}${tab === 'memories' ? `&status=${memoryStatus}&q=${encodeURIComponent(memoryQuery)}` : ''}`), enabled: tab === 'materials' || tab === 'memories' });
  ...
  return <section className={taskId ? 'space-y-4' : 'mx-auto max-w-5xl space-y-5 p-5 sm:p-7'}>
    ...
    <nav className="flex flex-wrap gap-2" aria-label="项目资料分类">{([['materials', '材料'], ['memories', '记忆'], ['brief', '当前简报'], ['search', '查找依据'], ...(taskId ? [] : [['tree', '知识树'], ['graph', '关系']])] as Array<[KnowledgeTab, string]>).map(([value, label]) => <Button key={value} size="sm" variant={tab === value ? 'secondary' : 'ghost'} onClick={() => setTab(value)}>{label}</Button>)}</nav>
    {tab === 'brief' ? <BriefPanel key={topicId} topicId={topicId || null} draft={briefDrafts[topicId] ?? null} updateDraft={updateBriefDraft} /> : tab === 'tree' ? <KnowledgeTree topicId={topicId || null} brief={<BriefPanel key={topicId} topicId={topicId || null} draft={briefDrafts[topicId] ?? null} updateDraft={updateBriefDraft} />} /> : tab === 'graph' ? <KnowledgeGraph topicId={topicId || null} /> : <>
      <div className="flex flex-wrap items-center gap-3">{tab === 'search' ? <Input aria-label="查找资料与记忆" placeholder="查找任务、材料与记忆" value={search} onChange={(event) => { setSearch(event.target.value); setCursor(0); }} className="max-w-md" /> : tab === 'memories' ? <><Input aria-label="搜索记忆" placeholder="搜索标题、正文或标签" value={memoryQuery} onChange={(event) => { setMemoryQuery(event.target.value); setCursor(0); }} className="max-w-sm" /><div role="group" aria-label="记忆状态" className="flex gap-1">{([['active', '活跃'], ['history', '历史'], ['all', '全部']] as const).map(([value, label]) => <Button key={value} size="sm" variant={memoryStatus === value ? 'secondary' : 'ghost'} aria-pressed={memoryStatus === value} onClick={() => { setMemoryStatus(value); setCursor(0); }}>{label}</Button>)}</div><span className="text-xs text-muted-foreground">结果 {list.data?.total ?? 0} 条</span><Button size="sm" onClick={() => openEditor()}>记录记忆</Button></> : <Button size="sm" onClick={() => openEditor()}>保存材料</Button>}{tab === 'materials' && <label className="flex items-center gap-2 text-xs text-muted-foreground"><input type="checkbox" checked={history} onChange={(event) => { setHistory(event.target.checked); setCursor(0); }} />包含历史与归档</label>}</div>
...
function ResourceEditor({ type, resource, topicId, taskId, onClose }: { type: 'materials' | 'memories'; resource?: Resource; topicId: string | null; taskId?: string; onClose: () => void }) {
  ...
  const [importance, setImportance] = useState(resource?.importance ?? 3);
  const [labels, setLabels] = useState(asLabels(resource?.labels).join(' '));
  const [origin, setOrigin] = useState(resource?.origin ?? 'manual');
  ...
  return <form className="glass-subtle space-y-3 rounded-xl border border-primary/30 p-4" onSubmit={(event) => { event.preventDefault(); void run(async () => {
    ...
    else { input.kind = kind; input.evidence = evidence; input.reason = reason || (resource ? '用户修订' : '人工记录'); input.importance = importance; input.labels = labels.split(/[\s,，]+/).map((label) => label.trim()).filter(Boolean); input.origin = origin; }
    ...
    <select aria-label={isMaterial ? '材料类型' : '记忆类型'} className="glass-control h-9 rounded-lg px-3 text-sm" disabled={Boolean(resource && (isMaterial || resource.kind === 'crystal'))} value={kind} onChange={(event) => setKind(event.target.value)}>{Object.entries(isMaterial ? { text: '文本', markdown: 'Markdown', link: '链接', thread: '保存会话', attachment: '附件' } : resource?.kind === 'crystal' ? { crystal: memoryKindNames.crystal } : memoryKinds).map(([value, name]) => <option key={value} value={value}>{name}</option>)}</select>
    {!isMaterial && <div className="flex flex-wrap items-center gap-3"><Stars value={importance} onChange={setImportance} /><select aria-label="记忆来源" className="glass-control h-9 rounded-lg px-3 text-sm" value={origin} onChange={(event) => setOrigin(event.target.value)}>{Object.entries(originNames).map(([value, name]) => <option key={value} value={value}>{name}</option>)}</select><Input aria-label="记忆标签" placeholder="标签，用空格或逗号分隔" value={labels} onChange={(event) => setLabels(event.target.value)} className="max-w-sm" /></div>}
